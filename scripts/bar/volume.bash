...
AUDIO_MED_THRESH=${AUDIO_MED_THRESH:-80}
...
function move_sinks_to_new_default {
  DEFAULT_SINK=$1
  pactl list sink-inputs short | cut -f1 | while read SINK; do
...
function set_default_playback_device_next {
  inc=${1:-1}
  sink_arr=($(pactl list short sinks | cut -f1))
  num_devices=${#sink_arr[@]}
  default_sink_index=$(pactl list short sinks | grep -n $(pactl get-default-sink) | cut -d: -f1)
  default_sink_index=$((($default_sink_index - 1 + $num_devices + $inc) % $num_devices))
...
function print_format {
  ...
  echo "$output" | envsubst '${SYMB}${VOL}'
}

function print_block {
  ACTIVE=$(pactl list sinks | grep -A 15 "State: RUNNING\|* index" | grep "State\|Name:\|Volume:\|Mute:")
  [ -z "$ACTIVE" ] && ACTIVE=$(pactl list sinks | grep -A 15 '* index' | grep "Name:\|Volume:\|Mute:")

  INDEX=$(echo "$ACTIVE" | grep "index" | grep -o '[0-9]\+' | head -n1)
  NAME=$(echo "$ACTIVE" | grep "Name:" | head -n1 | sed 's/.*: //; s/.*<\(.*\)>.*/\1/')
  VOL=$(echo "$ACTIVE" | grep "Volume:" | grep -o "[0-9]*%" | head -n1 | sed 's/%//')
  MUTED=$(echo "$ACTIVE" | grep "Mute:" | sed 's/.*: //')

  if [[ $USE_ALSA_NAME == 1 ]]; then
    ALSA_NAME=$(pactl list sinks | grep -A 20 "* index" | grep "alsa\." | grep "name" | head -n1 | sed 's/.*= "\(.*\)".*/\1/')
    NAME=${ALSA_NAME:-$NAME}
  elif [[ $USE_DESCRIPTION == 1 ]]; then
    DESCRIPTION=$(pactl list sinks | grep -A 20 "* index" | grep "device.description" | head -n1 | sed 's/.*= "\(.*\)".*/\1/')
